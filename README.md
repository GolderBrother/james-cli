...
在自己开发cli前，那肯定先要看些优秀的`cli`是如何实现的！虽然不是第一个吃螃蟹的，那也要想想怎么吃更好`^_^`

## 1.必备模块

我们先从大家众所周知的`vue-cli`入手,先来看看他用了哪些npm包来实现的
...
#2.4 链接全局包
设置在命令下执行james-cli时调用bin目录下的www文件
...
npm link
我们已经可以成功的在命令行中使用james-cli命令，并且可以执行main.js文件！
...
const actionsMap = {
  ...
  config: { // 配置配置文件
    ...
    alias: 'c',
    examples: [
      'james-cli config get <k>',
      'james-cli config set <k> <v>',
    ],
  },
  '*': {
...
执行james-cli create project,可以打印出 project
...
const fetchRepoList = async () => {
  ...
  const { data } = await axios.get('https://api.github.com/orgs/james-cli/repos');
  return data;
};
...
const fetchTagList = async (repo) => {
  const { data } = await axios.get(`https://api.github.com/repos/james-cli/${repo}/tags`);
  return data;
};
// 获取版本信息
...
const configFile = `${process.env[process.platform === 'darwin' ? 'HOME' : 'USERPROFILE']}/.jamesrc`; // 配置文件的存储位置
const defaultConfig = {
...
这样我们可以将create方法中所有的james-cli全部用获取到的值替换掉啦！
...
nrm use npm
npm publish # 已经发布成功～～
可以通过npm install james-cli -g 进行安装啦！